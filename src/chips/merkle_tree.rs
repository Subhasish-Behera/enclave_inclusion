...
impl MerkleTreeChip {
    ...
    pub fn configure(
        ...
        meta.create_gate("swap constraint", |meta| {
            ...
            let l_next = meta.query_advice(col_a, Rotation::next());
            let r_next = meta.query_advice(col_b, Rotation::next());

            // element_l_next = (element_r_cur - element_l_cur)*s + element_l_cur
            let swap_constraint_1 =s.clone() * (swap_bit.clone() * (b.clone() - l_next.clone()) + (Expression::Constant(Fp::from(1)) - swap_bit.clone()) * (a.clone() - l_next));

            // element_r_next = (element_l_cur - element_r_cur)*s + element_r_cur
            let swap_constraint_2 = s * (swap_bit.clone() * (a - r_next.clone()) + (Expression::Constant(Fp::from(1)) - swap_bit) * (b - r_next));
    ...
    pub fn swap_hashes_per_level(
        ...
        layouter.assign_region(
            ...
            |mut region| {
                ...
                //alternate swapping, not sure if map without collect works
                // if let Some(x) = swap_bit.value().copied() {
                //     if x != Fp::zero() {
                //         std::mem::swap(&mut l1_val, &mut r1_val);
                //     }
                // }
