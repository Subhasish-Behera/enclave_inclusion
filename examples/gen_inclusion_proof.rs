...
use summa_solvency::{
    circuits::{
        merkle_tree::MtInclusionCircuit,
...
fn main() {
    ...
    let circuit = MtInclusionCircuit::<LEVELS>::init(merkle_proof);
    println!("test222");
    println!("user_index: {:?}", user_index);
    // generate a universal trusted setup for testing, along with the verification key (vk) and the proving key (pk).
    let (params, pk, _) =
        generate_setup_artifacts(11, Some("/home/subhasishbehera/gnosis_inclusion/backend/ptau/hermez-raw-11"), circuit.clone())
            .unwrap();
    println!("test2222");
    ...
    // Save the verifier_solidity to a file
    let verifier_solidity_path = Path::new("verify.sol");
    let mut verifier_solidity_file = File::create(&verifier_solidity_path)
        .expect("Unable to create verify.sol file");
    verifier_solidity_file
        .write_all(verifier_solidity.as_bytes())
        .expect("Unable to write verifier_solidity to file");

    //println!("verifier_solidity: {:?}", verifier_solidity);
    ...
    let mut file = File::create("/home/subhasishbehera/gnosis_inclusion/examples/inclusion_proof_solidity_calldata2.json")
    ...
    let calldata_encoded = function
        ...
        .unwrap();
    //println!("calldata_encoded: {:?}", calldata_encoded);
    let mut evm = Evm::default();
    if !deployment_code.is_empty() {
        println!("Deployment code is non-empty. Size: {}", deployment_code.len());
    } else {
        println!("Deployment code is empty.");
    }
